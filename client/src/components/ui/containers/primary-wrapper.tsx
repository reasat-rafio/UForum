...
export const PrimaryWrapper: React.FC<PrimaryWrapperProps> = ({ children }) => {
  ...
  return (
    <section className="grid grid-cols-12 max-w-[1920px]">
      <div className="col-span-2  flex flex-col ">
        <div className="flex-1 py-[40%]">
          <div
            className="flex space-x-4 py-2 px-4 md:px-8 2xl:px-16 cursor-pointer"
          ...
          <div className="mt-5">
            <span className="py-2 px-4 md:px-8 2xl:px-16 text-gray-primary uppercase text-[12px] font-medium">
              Menu
            </span>
            <ul className="flex flex-col space-y-3">
              {menu.map(({ href, icon, name }) => (
                <Link key={name} href={href}>
                  <a className="flex space-x-4 hover:bg-secondary hover:bg-opacity-20 transition-all duration-300 hover:text-secondary py-2 px-4 md:px-8 2xl:px-16 relative group hover:border-l-[6px] border-secondary cursor-pointer text-black">
